...
Q6：我的棋子的toString方法黑白是用的color==Color.BLACK?"黑"，""白"判断的，是不是因为序列化和反序列化操作后造成color的引用改变，我把==改为equals方法后程序正常了(踩坑)

>您遇到的问题确实很可能是由于在序列化和反序列化过程中，枚举（`Color`）的引用比较（使用 `==`）与预期不符所导致的。在Java中，枚举类型的值在JVM中是单例的，即每个枚举常量在JVM中只有一个实例。因此，在大多数情况下，使用 `==` 来比较枚举常量是安全的，因为每个枚举常量都指向JVM中相同的对象。
>
>然而，在序列化和反序列化过程中，虽然枚举常量的值在逻辑上保持不变，但它们的物理引用（即内存地址）可能会发生变化。这是因为序列化机制会保存枚举常量的名称（而不是其内存地址），并在反序列化时根据这些名称重新创建枚举常量。因此，在反序列化后得到的枚举常量对象，虽然逻辑上与原对象相同，但在物理上可能是JVM中的另一个实例。
>
>这就是为什么在序列化/反序列化后，使用 `==` 来比较枚举常量可能会失败的原因。而使用 `equals()` 方法则不会受到这种影响，因为 `equals()` 方法在枚举类型中被重写为比较枚举常量的名称（或其他逻辑上唯一标识枚举常量的属性），而不是它们的物理引用。
>
>因此，将 `==` 改为 `equals()` 方法后程序恢复正常是完全合理的。这是一个更健壮、更可靠的做法，因为它不依赖于枚举常量的物理引用是否保持不变。在编写涉及序列化和反序列化的代码时，建议总是使用 `equals()` 方法来比较枚举常量，以避免潜在的问题。
>
>
